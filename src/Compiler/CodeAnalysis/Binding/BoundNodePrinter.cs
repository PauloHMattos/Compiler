...
namespace Compiler.CodeAnalysis.Binding
{
    internal static class BoundNodePrinter
    {
        ...
        private static void WriteNestedExpression(this IndentedTextWriter writer, int parentPrecedence, BoundExpression expression)
        {
            ...
            {
                writer.WriteNestedExpression(parentPrecedence, unary.Operator.SyntaxKind.GetUnaryOperatorPrecedence(), unary);
            }
            else if (expression is BoundBinaryExpression binary)
            {
                writer.WriteNestedExpression(parentPrecedence, binary.Operator.SyntaxKind.GetBinaryOperatorPrecedence(), binary);
            }
            else
            {
        ...
        private static void WriteIfStatement(BoundIfStatement node, IndentedTextWriter writer)
        {
            ...
            {
                writer.WriteKeyword(SyntaxKind.ElseClause);
                writer.WriteLine();
        ...
        {
            var precedence = node.Operator.SyntaxKind.GetUnaryOperatorPrecedence();
